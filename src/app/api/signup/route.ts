...
import { createSSRClient } from "@/utils/supabase/server"; // Adjust the path as necessary
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!authData.user) {
      return new NextResponse(
        JSON.stringify({ message: "User not created successfully." }),
        { status: 404 }
      );
    }
    ...
    return new NextResponse(
      JSON.stringify({
        ...
        user: { id: authData.user.id, username, full_name },
      }),
      { status: 201 }
    );
  } catch (error: unknown) {
