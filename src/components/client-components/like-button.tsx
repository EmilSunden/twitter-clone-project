...
const LikeButton = ({
  ...
  return (
    <button
      ...
      className="rounded-full text-white space-x-2 flex items-center hover:bg-white/10 transition duration-200 cursor-pointer p-2"
    >
      {isUserHasLiked ? (
        <AiFillHeart className="w-5 h-5 text-red-600" />
      ) : (
        <AiOutlineHeart className={cn("w-5 h-5")} />
      )}
